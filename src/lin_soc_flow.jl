...
function Solve_Linearized_Dist_SOC(buses, lines, generators, fp0, fq0, l0)
info("Starting Linearized DistSOC Model")
...
# All buses without root
    ...
    @constraint(m, -fp0[b]^2 - fq0[b]^2 + 2fp0[b]*fp[b] + 2fq0[b]*fq[b] <= lines_to[b].s_max^2)
    @constraint(m, term_p^2 + term_q^2 +
                     2*(fp[b] - fp0[b])*term_p +
                     2*(fq[b] - fq0[b])*term_q +
                     2*(l[b] - l0[b])*(term_p*R + term_q*X) <= lines_to[b].s_max^2)
    # Same as above but rearranged
    # @constraint(m, -1*(fp0[b] + l0[b]*R)^2 - (fq0[b] + l0[b]*X)^2 + 
    #                 2*(fp0[b] + l0[b]*R)*fp[b] + 2*(fq0[b] + l0[b]*X)*fq[b] +
    #                 2*(l0[b]*R^2 + fp0[b]*R + l0[b]*X^2 + fq0[b]*X)*l[b] <= lines_to[b].s_max^2) 
    
    @constraint(m, v[b_ancestor] - 2*(R*fp[b] + X*fq[b]) - l[b]*(R^2 + X^2) == v[b])
    @constraint(m, ((2*fp0[b])/l0[b]) * fp[b] + ((2*fq0[b])/l0[b]) * fq[b] - ((fp0[b]^2 + fq0[b]^2)/l0[b]^2) * l[b] <= v[b])
end
